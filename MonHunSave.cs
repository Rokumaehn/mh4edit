...
namespace mh4edit
{
   public class MonHunSave
   {
      ...
      public void Save()
      {
         ...
         BitConverter.GetBytes((ushort)16).CopyTo(Buffer, 0);
         BitConverter.GetBytes((ushort)Key).CopyTo(Buffer, 2);
         BitConverter.GetBytes(csum).CopyTo(Buffer, 4);
         ...
         Blowfish bf = new Blowfish(Encoding.ASCII.GetBytes("blowfish key iorajegqmrna4itjeangmb agmwgtobjteowhv9mope"));
         bf.Encipher(Buffer, Buffer.Length);
